    ...
    ILogger<ContasReceberSyncService> logger) : ISyncService
{
    ...
    {
        logger.LogInformation("Cancelando TituloReceber OmieId: {OmieId} via soft-delete.", omieId);
        var existing = await dbContext.TitulosReceber.FirstOrDefaultAsync(t => t.OmieId == omieId, ct);
    ...
    private static TituloReceber MapToEntity(OmieContaReceber omie, Cliente? cliente, Vendedor? vendedor, ContaCorrente? contaCorrente) =>
        new()
        {
            Id = Guid.NewGuid(),
    ...
    private static void AtualizarEntidade(TituloReceber existing, OmieContaReceber omie, Cliente? cliente, Vendedor? vendedor, ContaCorrente? contaCorrente)
    {
