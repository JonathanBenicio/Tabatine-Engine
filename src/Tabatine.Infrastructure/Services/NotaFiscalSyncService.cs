...
{
    public class NotaFiscalSyncService(
        IOmieClient omieClient, 
        AppDbContext dbContext, 
        ISyncStateRepository syncState, 
        ILogger<NotaFiscalSyncService> logger,
        IDistributedLockService lockService) : ISyncService
    {
        private const string EntityName = "notafiscal";
        ...
        public async Task SyncByIdAsync(long omieId, CancellationToken ct = default)
        {
            ...
            if (lockToken == null)
            {
                logger.LogWarning("Não foi possível adquirir trava para a Nota Fiscal {OmieId} após espera. Abortando sync individual.", omieId);
                return;
            }
        ...
        private async Task ProcessNotaFiscalBatchAsync(List<OmieNotaFiscal> notasFiscaisOmie, IReadOnlyDictionary<long, string>? preAcquiredLocks, CancellationToken ct)
        {
            ...
            try 
            {
                foreach (var nf in notasFiscaisOmie)
                {
                    ...
                    var lockToken = await AcquireResourceLockAsync(omieIdNf, ct, wait: false);
                    if (lockToken == null)
                    {
                        logger.LogInformation("Nota Fiscal {OmieId} já está sendo processado por outro worker. Pulando no lote.", omieIdNf);
                        continue;
                    }
                    activeLocks[omieIdNf] = lockToken;
                }

                List<long> omieNfIds = notasFiscaisOmie.Where(n => activeLocks.ContainsKey(n.Compl.IdNf)).Select(n => n.Compl.IdNf).ToList();
                List<long> omieClienteIds = notasFiscaisOmie.Where(n => activeLocks.ContainsKey(n.Compl.IdNf)).Select(n => n.Destinatario.CodigoCliente).Distinct().ToList();
                List<long> omiePedidoIds = notasFiscaisOmie.Where(n => activeLocks.ContainsKey(n.Compl.IdNf) && n.Compl.IdPedido.HasValue).Select(n => n.Compl.IdPedido!.Value).Distinct().ToList();

                List<long> omieVendedorIds = notasFiscaisOmie
                    .Where(n => activeLocks.ContainsKey(n.Compl.IdNf) && n.Titulos != null)
                ...
                foreach (OmieNotaFiscal omieNf in notasFiscaisOmie)
                {
                    ...
                    if (!activeLocks.ContainsKey(omieId)) continue;
