...
namespace Tabatine.Infrastructure.Services
{
        ...
        IDistributedLockService lockService) : ISyncService
    {
        ...
        private async Task ProcessPedidosBatchAsync(List<OmiePedido> pedidosOmie, CancellationToken ct, Dictionary<long, string>? preAcquiredLocks = null)
        {
            ...
            try 
            {
            ...
            } // Fim do foreach pedidosOmie
                
                int retries = 0;
                const int maxRetries = 2; // Reduzido pois o Lock deve resolver a maioria
                ...
                while (retries < maxRetries)
                {
                    ...
                    {
                        retries++;
                        logger.LogWarning(ex, "Concorrência residual detectada ao salvar pedidos. Tentativa {Retry}.", retries);
                        ...
                        foreach (var entry in ex.Entries)
                        {
                            ...
                            else entry.OriginalValues.SetValues(databaseValues);
                        }
                    }
                }
            }
