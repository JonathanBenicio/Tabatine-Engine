...
/// <summary>
/// Webhook handler for ContaPagar events.
/// </summary>
public class ContasPagarWebhookHandler(ContasPagarSyncService syncService, IEnumerable<INotificationService> notificationServices) : IWebhookEventHandler
{
    public IEnumerable<string> SupportedEvents => new[]
    {
        "Financas.ContaPagar.Incluido",
    ...
    public async Task HandleAsync(WebhookEvent webhookEvent, CancellationToken cancellationToken)
    {
        ...
        if (webhookEvent.Event.EndsWith(".Excluido", StringComparison.OrdinalIgnoreCase))
        {
            ...
            title = "Conta a Pagar Excluída (Cancelada)";
            summary = $"Título de Pagar ID {omieId.Value} marcado como CANCELADO localmente.";
        }
        else
        {
            await syncService.SyncByIdAsync(omieId.Value, cancellationToken);
            title = webhookEvent.Event.EndsWith(".Incluido") ? "Nova Conta a Pagar" : "Conta a Pagar Alterada";
            summary = $"Título de Pagar ID {omieId.Value} sincronizado com sucesso.";
        }
